...
$que=mysql_query("INSERT INTO articulos_index (categoria,titulo,subtitulo,contenido,publicado,autor,tag) values ('{$categoria}','{$titulo}','{$subtitulo}','{$contenido}','{$publicado}','{$autor}','{$tag}') ",$link);
			if(!$que){
	...
	if (!((strpos($tipo_archivo, "png") || strpos($tipo_archivo, "jpeg")) && ($tamano_archivo < 1500000))) { 
	   	...
	   	if (move_uploaded_file($_FILES['imagen']['tmp_name'], $path.$nombre_archivo)){ 
			$que=mysql_query("INSERT INTO articulos_index (categoria,titulo,subtitulo,contenido,publicado,autor,imagen,tag) values ('{$categoria}','{$titulo}','{$subtitulo}','{$contenido}','{$publicado}','{$autor}','{$nombre_archivo}','{$tag}') ",$link);
					if(!$que){
						die ("Pos no se capturó el contenido, parece que: " .mysql_error());
						echo '<script>window.location.href="../../articulos.php?ruta=if_articulos.php&capturado=0";</script>';				
					}else{
