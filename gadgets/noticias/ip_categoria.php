...
if(empty($nombre_archivo)){
	$que=mysql_query("INSERT INTO noticias_categoria (nombre) values ('{$nombre}')",$link);
	if(!$que){
		die ("Pos no se capturó el contenido, parece que: " .mysql_error());
		echo '<script>window.location.href="../../noticias.php?ruta=if_categoria.php&capturado=0";</script>';				
	}else{
	...
	if (!((strpos($tipo_archivo, "png") || strpos($tipo_archivo, "jpeg")) && ($tamano_archivo < 1500000))) { 
			echo '<script>window.location.href="../../noticias.php?ruta=if_categoria.php&capturado=2";</script>';  
	}else{ 
	   	if (move_uploaded_file($_FILES['imagen']['tmp_name'], $path.$nombre_archivo)){ 
			$que=mysql_query("INSERT INTO noticias_categoria (nombre,imagen) values ('{$nombre}','{$nombre_archivo}')",$link);
			if(!$que){
				die ("Pos no se capturó el contenido, parece que: " .mysql_error());
				echo '<script>window.location.href="../../noticias.php?ruta=if_categoria.php&capturado=0";</script>';				
			}else{
