...
if(empty($nombre_archivo)){
	$que=mysql_query("UPDATE fotos_categoria SET  nombre = '$nombre',imagen = '$imagen',belong = '$belong' WHERE id = '$rubro'" ,$link);
	if(!$que){
	...
	if (!((strpos($tipo_archivo, "png") || strpos($tipo_archivo, "jpeg")) && ($tamano_archivo < 1500000))) { 
	   	...
	   	if(move_uploaded_file($_FILES['imagen']['tmp_name'], $path.$nombre_archivo)){ 
			$que=mysql_query("UPDATE fotos_categoria SET  nombre = '$nombre',imagen = '$nombre_archivo',belong = '$belong' WHERE id = '$rubro'" ,$link);
			if(!$que){
				die ("Pos no se capturó el contenido, parece que: " .mysql_error());
				echo '<script>window.location.href="../../fotos.php?ruta=if_fotos.php&capturado=0";</script>';				
			}else{
...
?>
/*
$mysql=mysql_query("UPDATE fotos_categoria SET  nombre = '$nombre',imagen = '$imagen',belong = '$belong' WHERE id = '$rubro'" ,$link);
if(!$mysql){die ("Pos no se capturó el contenido, parece que: " .mysql_error());
}else{
echo '<script>window.location.href="../../fotos.php?ruta=if_categoria_a.php&capturado=1";</script>';
}
?>
